#!/usr/bin/env bash
# SkillOpt round-2: fresh D_tr traces after loop-break, new output dir, strict D_sel gate.
set -euo pipefail
...
env -u LD_LIBRARY_PATH \
  ...
  ROBOAGENT_MAX_AW_STEPS=0 \
  ROBOAGENT_OG_BACKEND=llmdet_qwen_usr ROBOAGENT_EG_BACKEND=qwen ROBOAGENT_SD_BACKEND=usr \
  ...
  "$ENV_BIN/python" -u run_aw.py --qwen_path "$CKPT" \
    --save_path "$OUT/dev" --split eval_in_distribution --start 0 --end 20 --seed 42 \
  >> "$DEV_LOG" 2>&1

"$ENV_BIN/python" "$CODE/training/thor_cleanup.py" KILL || true
...
echo $! > "$OUT/pid"
echo "SKILLOPT_ROUND2_PID=$(cat $OUT/pid) GPU=$GPU OUT=$OUT"
